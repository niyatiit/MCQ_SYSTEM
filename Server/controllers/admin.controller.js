...
const exportStudentsPDF = asyncHandler(async (req, res) => {
  ...
  res.setHeader('Content-Disposition', 'attachment; filename=student-results.pdf');
  ...
  doc
    ...
    .fontSize(20)
    .font('Helvetica-Bold')
    .text('MCQ Examination System', marginLeft, 18, { width: contentWidth, align: 'center' });
  doc
    .fontSize(12)
    .font('Helvetica')
    .text('Student Exam Results Report', marginLeft, 44, { width: contentWidth, align: 'center' });
  ...
  const semLabel = filter.semester ? `Semester ${filter.semester}` : 'All Semesters';

  const boxY = 90;
  doc.rect(marginLeft, boxY, contentWidth, 30).fill('#eff6ff').stroke('#1e3a8a');
  doc
    .fillColor('#1e3a8a')
    .font('Helvetica-Bold')
    .fontSize(11)
    .text(`Department: ${deptLabel}      |      ${semLabel}      |      Generated: ${new Date().toLocaleString()}`, marginLeft + 10, boxY + 9);
  ...
  const headers = ['Name', 'Enrollment No.', 'Department', 'Semester', 'Subject', 'Status', 'Marks', 'Submitted At'];
  ...
  students.forEach((s, idx) => {
    ...
    row.forEach((cell, i) => {
      // color status text
      if (i === 5) {
        const statusColors = { completed: '#16a34a', in_progress: '#ca8a04', not_started: '#6b7280' };
  ...
  doc
    .fontSize(8)
    .fillColor('#6b7280')
    .text(`Total Records: ${students.length}`, marginLeft, doc.page.height - 30, { align: 'left' });
